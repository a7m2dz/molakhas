...
export function GET() {
  ...
  // Google News wants recent published URLs. Keep provenance/source independent
  // from indexability so approved editorial stories are included as well.
  const recent = stories
    .filter((s)=>s.status==='approved' && +new Date(s.publishedAt)>=cutoff)
