...
export function GET() {
  ...
  const urls = recent.map(s=>{
    const image = s.image?.src || `/news-images/${s.slug}.webp`;
    return `<url><loc>${esc(`${base}/${s.section}/${s.slug}`)}</loc><news:news><news:publication><news:name>ملخص</news:name><news:language>ar</news:language></news:publication><news:publication_date>${new Date(s.publishedAt).toISOString()}</news:publication_date><news:title>${esc(s.title)}</news:title></news:news><image:image><image:loc>${esc(`${base}${image}`)}</image:loc><image:title>${esc(s.image?.alt || s.title)}</image:title></image:image></url>`;
  }).join('');
