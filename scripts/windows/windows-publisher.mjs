...
async function main() {
  if (!String(process.env.OMNIROUTE_API_KEY || '').trim()) {
    throw new Error('OMNIROUTE_API_KEY is missing from the Windows user environment.');
  }

  log(`Windows publisher cycle start; model=${process.env.OMNIROUTE_MODEL}; auth=stored-user-key`);
